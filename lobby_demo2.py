...
class SerialDisplayer:
    def __init__(self, use_grip_cmds, CP210x_only, no_input=False, reverse=False, camera_capture=0, fade_rate=20):
        ...
        if not self.input_listener:
            ...
            self.input_listener = serial.Serial(ir_port,'460800', timeout=1)
    ...
    def wave_hand(self, fpos):
        ...
        for serial in self.slist:
            try:
                ...
                print(f"Port {serial.port} error, attempting to reconnect...")
                serial.close()
                time.sleep(0.5)
                serial.open()             
        ...
        with mp_hands.Hands(
            ...
            while True:
                ...
                if time.time() - start_time > 1800:
                    print("One hour passed. Resetting serial connections...")
                    for i in range(len(self.slist)):
                        try:
                            self.slist[i].close()
                            time.sleep(.5)
                            self.slist[i].open()
                            print(f"Reopened serial port: {self.slist[i].port}")
                        except Exception as e:
                            print(f"Error resetting port {self.slist[i].port}: {e}")
